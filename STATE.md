...
5. 확인: `tool/checks/test` 녹색(일회용 postgres:18 → `db/migrate.sh` 2회 → 테스트), `git config core.hooksPath` = `.githooks`
...
## 3. 경계
- **사전 승인됨(2026-08-23, 에픽 이슈에 정본)**: 2단계 DB 행 변경 · 002+ DDL 운영 적용(추가만, DROP·타입변경·데이터손실 제외) · LLM(`CLAUDE_API_KEY`, 하드스톱 $7, 리뷰·댓글 원문 전송 OK) · push·이슈·gh · 1차/2차 패스 기준(계약) · STATE.md 체계.
- **승인 필요(매번)**: 기존 스택 컨테이너 재빌드·정지·재시작, tubedepth watch 재가동, 5단계 컷오버(에픽 이슈의 조건 참조).
- **금지**: archive 구 레포 수정(로컬 stack·data-portal 설정 편집은 예외, 푸시 없음) · 구 cosmai 플랫폼 확장 · secret 값 출력·`.env` 커밋 · `--no-verify`·force push · 새 수집(컷오버 전; 그림자 실행은 에픽 조건으로만).
