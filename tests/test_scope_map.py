"""#215: the risk classifier is only as honest as `tests/scope.toml`.
...
def test_the_full_suite_list_names_paths_that_are_really_there():
    # An entry with a typo in it silently stops forcing the full suite for the path it meant.
    missing = [path for path in scope()["full"] if not (REPO_ROOT / path).exists()]  # type: ignore[union-attr]
