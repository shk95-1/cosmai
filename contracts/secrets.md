...
- 컴포넌트가 읽는 키:
  ...
  - analysis/polarity (LLM): `CLAUDE_API_KEY` (Anthropic API 키; SDK 기본 env 이름이 아니므로 코드가 명시적으로 넘긴다). 예산 하드스톱은 코드 상수 `LLM_BUDGET_USD = 7.0`, 누적은 `needs.llm_usage`(DDL 002).
  - 선택: `OLLAMA_URL` (기본 `http://localhost:11434`; 배관 테스트 전용, 채택 판정에 쓰지 않는다)
