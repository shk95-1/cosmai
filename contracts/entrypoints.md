...
- **불용어 목록도 조사 목록도 두지 않는다**(포크 #37, ydc `lexicon.json` 처분). 일반어는 `terms` 의 lift
  축이 걷어내고(`analysis/retrieval/terms.py`), 조사는 Kiwi 의 태그가 가른다 — `bm25.tokenize` 는
  `KIWI_TAGS` 밖(조사 `J*`·어미 `E*`)을 버리고 한 글자 명사도 버려서, ydc 가 코퍼스 실측으로 검증한
  조사 30개를 어간에 붙여도 토큰이 하나도 달라지지 않는다(실측 2026-08-26, 30/30 ·
  `tests/retrieval/test_particles.py`). 손으로 관리하는 목록을 되살리면 버전을 못 받는 두 번째 사전이
  된다(포크 #8). 별칭은 반대다 — 목록이 아니라 버전을 받는 행으로 산다: 주제 표기는 위 사전
  (`ruleset='retrieval-topic'`), 브랜드 표기는 `needs.entity_lexicon`(`formats.md` §사전 CSV).
