...
같은 락을 쓰는 주인의 극성 패스(2.5~4시간)와 겹치면 뒤에 온 쪽이 그 밤을 통째로 건너뛰므로, 그 패스에
크론 줄이 생기는 날 그 줄과 `0 5` 사이의 간격은 패스의 최악 소요보다 넓어야 한다 — 오늘 숫자로 `0 0`
(간격 5h > 4h)은 되고 `0 2`(3h)는 안 된다. 계산은 `stack/crontab.d/analyze` 에 적혀 있다.
