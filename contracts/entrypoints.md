...
- **색인·추출 축에는 불용어 목록도 조사 목록도 두지 않는다**(포크 #37, ydc `lexicon.json` 처분). 그 축은
  ...
  (`formats.md` §사전 CSV).
  `lexicon.json` 의 별칭 9개는 **다 판정됐다**(포크 #56): 셋은 이미 있었고, `선크림추천` 은 확장이 잡아
  행이 필요 없으며, 넷(`썬쿠션`·`썬스틱`·`sunscreen`·그리고 후보에서 온 `선에센스`·`선스프레이`)은 주제
  사전 v3 의 행이 됐고, `올영` 은 자리가 있어도 소비자가 없어 미등재다. 판정 원장과 등재 기준 셋은
  `formats.md` §주제 사전 v3 이고, 그 df 를 다시 재는 길은 `tool/measure-lexicon-candidates` 다.
