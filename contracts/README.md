...
| `ddl/needs/022_panel_and_quarter.sql` | 패널 모집단 `needs.panel_channel`(43채널 명부의 자리 — 값은 포크 #31)과 분기 입자의 정본 `needs.metrics_topic_quarter`(월과 **공존**한다; 어느 표가 어느 입자를 지는지는 `formats.md` §시간). 두 표 다 어떤 모집단에 대한 비율인지를 행에 남긴다 | `tests/test_panel_quarter_contract.py` (정본·명부·분모·역할 어휘·`quarter` 문법) + `tests/test_month_grain_regression.py` (월 산출 불변) + `tests/test_ddl_additive_only.py` |
