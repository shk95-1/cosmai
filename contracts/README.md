...
| `ddl/needs/022_panel_and_quarter.sql` | 패널 모집단 `needs.panel_roster`+`needs.panel_channel`(43채널 명부의 자리 — 값은 포크 #31)과 분기 입자의 정본 `needs.metrics_topic_quarter`(월과 **공존**한다; 어느 표가 어느 입자를 지는지는 `formats.md` §시간). 두 표 다 어떤 모집단에 대한 비율인지를 행에 남기고, 그 포인터는 판본 표를 FK 로 가리킨다. 주제 축은 `topic_key` 다 — 레지스트리가 `aspect_lexicon(ruleset='retrieval-topic')` 이라 `needs.need_key` 와 다른 축이다. 행 집합의 두 불변식(조밀한 격자 · 닫힌 분모)은 뷰 `db/views/metrics_topic_quarter_violation.sql` 이 되묻는다 | `tests/test_panel_quarter_contract.py` (정본·명부·분모·역할 어휘·`quarter` 문법·두 축·자리수·불변식 뷰) + `tests/test_month_grain_regression.py` (월 산출 불변) + `tests/test_ddl_additive_only.py` |
