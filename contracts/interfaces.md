...
- 한계를 카드 안에 넣는다(설계 원칙 4): `hold_reason` · `single_source` · 최근 분기 과소 집계 · 일반어
  별칭. ydc 가 함께 실은 자막 이득과 교차 검증 셋(커머스 구성비·설문 긍정률·제품 구성)은 그 원천이 이
  레포에 없어 빠졌다 — 빠졌다는 사실은 이 줄에 남고, 카드에 빈 절로 남지 않는다.
...
갔는지 항상 말할 수 있다). **승격하지 않는다.** 네 갈래 중 셋이 우리에게 원천이 없고, 그 자리에 지금 있는
유일한 목록을 쓰면 ydc 가 밟은 함정을 그대로 밟기 때문이다.
...
| `needs.entity_lexicon` `kind='ingredient'` v1 | 43행 / 28키 / 고유 42표기 (원본 `eval/lexicon/ingredient_kr_colloquial_v1.csv`) | `source='paper_lexicon'` — 논문 검색어 어휘다. `category` 열이 uv_filter·spec·mechanism·anatomy·regulatory 로 갈리고 `category='ingredient'` 는 **8키**뿐이며, `무기자차` 를 ZINC_OXIDE 와 TITANIUM_DIOXIDE **양쪽**의 표기로 담는다 |
...
| `bm25` | 브랜드 | `needs.entity_lexicon` `kind='brand'`(`eval/lexicon/brand_lexicon_v1.csv` 859행) | 열려 있다 |
| `bm25` | 등록번호(10자리 보고번호) | 없다 | **막힘** — 포크 #55 |
| `bm25` | SPF/PA 표기 | 정규식 하나 | 열려 있다 |
| `temporal_filter` | 시점 표현 + `report_date` | 없다 | **막힘** — 포크 #55 (`mfds_items.csv` 는 `COSMETIC_REPORT_SEQ`·`ITEM_NAME`·`ENTP_NAME`·`report_date` 넉 칸이라 **성분 축은 열지 않는다**) |
| `multi_source` | 정확 신호 + 자연어 신호 | 성분명 축이 서야 갈린다 | **막힘** (아래) |
| `vector` | 그 외 | — | 열려 있다 |

브랜드와 SPF/PA 만으로는 라우터가 서지 않는다 — 그 둘만 보는 라우터는 `백탁 관련해서 소비자들이 뭐라고 해`
와 `에칠헥실트리아존` 을 **같은 갈래(vector)** 로 보내고, 그것은 라우팅이 아니다.

### 이진으로 갈리지 않는 자리는 우리에게도 있다 (`multi_source`)
ydc 의 `콜라겐 들어간 제품 뭐가 좋아` — `콜라겐` 은 진짜 성분이면서 담론어다(담론/제품 비율 콜라겐 281 ·
판테놀 7). 우리 자리는 활성 주제 사전이 그대로 가리킨다: `mfds_inci` 표기 24개 중 **15개**가 토크나이저
사전에도 있고, 그 15개는 무기·유기자차 주제의 별칭이자 실제 자외선차단 성분이다. 정확 신호와 자연어 신호가
...
| 활성 주제 사전의 `ko` 별칭 67개 중 토크나이저 사전에 있는 것 | **11** |
...
- **ydc 의 7/10 은 재현되지 않았다(4/10). 함정은 그대로 재현됐다.** 그 둘은 다른 말이다: 비율은 표본의
  ...
  15번째다) — 넣으면 오르고, 빼면 내린다. **그래서 이 절의 답은 첫 줄이 아니라 넷째·다섯째 줄이다**:
...
넘기는 것은 하나뿐이다: **라우터는 #11 을 대체하지 못한다** — 네 갈래 중 셋이 막혀 있어 지금 세울 수 없고,
성분표가 오기 전에는 그 상태가 바뀌지 않는다. 그러니 #11 은 기본값을 골라야 한다.
**위 표의 다섯 줄은 어느 것도 #11 의 입력이 아니다** — 기본 엔진 판단의 입력은 전 소스에서 같은 자로 잰
