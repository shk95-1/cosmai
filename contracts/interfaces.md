...
- **선별 규칙 넷** (ydc `evidence_comments.py` 의 규칙이고, 순서가 아니라 넷이 모두 걸린다):
  ...
  2. **제작자 본인 댓글은 뺀다.** 그 영상 채널의 해시(`sha256('youtube:' || channel_id)` 앞 24자, 수집기가
     ...
     좋아요 상위가 대부분 고정 댓글(타임라인·인사말·요약)이라 소비자 발화가 아니다. 픽스처 댓글 418건
     중 **15건**이 여기서 빠진다.
  ...
  4. 순서는 `like_count` 내림차순이고 **동점은 `doc_id`** 다. 2차 키가 계약인 것은 ydc 가 CSV 읽기 순서에
     ...
     재실행이 같은 행을 내야 하므로 그 자리를 비워 둘 수 없다: 픽스처 46셀 중 **32셀**에 동점이 있고,
  ...
  - 셀당 상한은 `TOP_PER_CELL = 3`. 카드 한 장에 들어가는 수라 025 의 CHECK 이 아니라 여기가 그 자리다 —
    DDL 은 추가만이라 한번 적은 상한을 되돌릴 수 없다.
...
실측 (2026-08-26 · 픽스처 코퍼스 댓글 418건 = 청크 439개 · 질의는 주제 별칭 61개 · `analysis/retrieval/bm25`
의 색인과 `analysis/retrieval/eval` 의 채점, 정답은 `corpus_mention`):

| 무엇을 재는가 | 값 |
|---|---|
| BM25 top-10 이 `corpus_mention` 의 답과 겹치는 정도 | P@10 **.604** · MRR@10 .601 · Hit@10 65.6% |
| 고른 근거 102행이 그 주제 **어느 별칭의** top-10 안에라도 드는 비율 | **78/102 = 76.5%** |

두 줄이 같은 말을 한다: 검색으로 갈아끼우면 근거 넷 중 하나가 사라진다. 아래 별칭 합집합(주제마다 최대
10질의 × 10건)이라는 후한 조건에서 잰 값이라 top-10 하나로 좁히면 더 내려간다.

**엔진 선택**: 이 용도는 셋 중 **하나도 쓰지 않는다.** 근거 수집은 §검색 실측 의 두 모드 중 literal 쪽이고
(질의가 곧 주제 별칭이라 정답 문서에 그 글자가 있다) 그 모드의 승자인 bm25 조차 위 표만큼만 맞힌다 —
답을 이미 `corpus_mention` 이 갖고 있기 때문이다. S6 이 #11 에 주는 것은 기본 엔진의 근거가 아니라 그
반대다: **근거 수집이라는 구체적 용도가 검색의 순위를 필요로 하지 않는다.** 검색이 자기 자리를 갖는 것은
사전에 없는 말(`백탁` 이 아니라 `허옇게 떠요`)로 코퍼스를 뒤질 때이고, 그 자리는 heldout 모드이며 벡터가
유일하게 0 을 넘은 자리다(§검색 실측).
