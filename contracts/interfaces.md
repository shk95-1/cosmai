...
| 팔 | 무엇 | 왜 |
|---|---|---|
| `seen` (기존) | `doc_id` 가 청크 색인에 있는 리뷰 | 우리 §대조·§검색 이 실제로 센 그 리뷰들이다 |
...
|---|---|---|
| `Prefer: count=exact` 뒤 행수 대조 | **한 트랜잭션 스냅샷**(`REPEATABLE READ`) | 두 읽기가 같은 시점을 본다. 세어 놓고 다시 세는 것은 이 자리에서 항등식이라 검사가 아니다 |
...
— 승격이 답을 바꾸지 않았다는 말이 서려면 같은 축이어야 한다. 구성비 축에서 몇 주제가 재현되는지는
`note` 가 따로 센다.

**순위에는 게이트가 있다.** 기존 팔의 문서 수가 `MIN_MENTIONS`(5) 미만인 주제는 순위를 갖지 않는다
(`None`) — 13주제 축의 꼬리는 0으로 묶여 있어 그 자리에서 매긴 등수는 동률 정렬의 산물이지 순위가 아니다.
ydc 는 다섯 주제가 전부 값을 가져 이 게이트가 필요 없었다.

### 갈리면 왜 갈리는가 — 세 갈래를 각각 잰다
비율이 통째로 움직였을 때 ydc 가 실제로 밟은 순서다. **원인을 셋으로 갈라 각각 재고, 답이 남는 것을 본다.**

- **창** (`WindowReading`) — 홀드아웃이 **새 기간**인가 **같은 창이 길어진 것**인가. 두 팔의 `captured_at`
...
- **플랫폼 구성** (`PlatformRow` · `StandardRow`) — 수준이 통째로 오르는 두 번째 원인은 플랫폼 구성이
  바뀐 것이다(다이소몰 리뷰는 선크림 표현이 거의 없어서 그 비중이 흔들리면 전체가 따라 움직인다).
  플랫폼별로 갈라 세고 **기존 팔의 구성비로 홀드아웃을 재가중한다** — 구성 효과를 뺀 값(`standardized`)과
  기존 값의 차(`residual_pp`)가 남는 것이 실제 변화다. 가중치는 **기존 팔에 있는 플랫폼**의 것이므로
  홀드아웃에만 있는 플랫폼은 이 가중합에 들지 않는다(그 사실은 `PlatformRow` 가 0으로 싣는다).
...
**재현 실패는 발견이지 실패가 아니다.** 셋 다 종료 코드 0 이고, 어느 갈래인지는 `note` 와 표가 싣는다 —
...
| `RANK_TOP` | `2` | 같은 함수의 `ra[:2] == rb[:2]` | 채택. **뒤에 붙은 `and ra[-1] == rb[-1]`(최하위 일치)는 옮기지 않는다** — ydc 의 다섯 주제는 전부 값을 가졌지만 우리 13주제 축의 꼬리는 0으로 묶여 최하위가 동률이라, 그 자리를 검사하면 정렬 순서를 검사하게 된다 |
...
- **ydc 출처는 셋 다 `v0.3.0:vector_threshold.py` 에서 확인했다**(2026-08-27, 읽기만):
  ...
  (:149 과 :164), 우리가 첫 갈래를 뺀 것이다. `analysis/slices/ydc/` 는 `v0.1.0` 사본이라 이 파일이 없어
  이 레포 안에서는 확인되지 않는다.
