...
구성비 축에서 몇 주제가 재현되는지는 `note` 의 `share_reproduced=` 가 따로 센다. **그 셈이 쓰는 문턱도
`MATERIAL_PP` 이지만 그것은 판정이 아니라 읽기 보조다** — 문턱은 ydc 가 언급률 위에서 고른 수이고 구성비
축에서 다시 적합한 것이 아니다. 두 수를 빼거나 더하지 않고, 같은 분모(`ranked`) 위의 다른 축으로만
나란히 놓는다.
