...
- 이 표는 §검색 실측 과 **같은 저장소·같은 질의 목록** 위에 서 있다. 다만 **나란히 읽는 수는 아니다** —
  ...
  (실측 2026-08-27 · 다시 재는 길은 `cosmai lexicon diff --kind aspect`).
...
### 그러면 근거 없는 질의는 무엇이 막는가 — 문서빈도 (`analysis/retrieval/grounding.py`)
코사인은 안 갈리지만 **df 는 갈린다.** 규칙 하나다: **길이 4 이상인 질의 토큰 중 문서빈도가 0 인 것이
...
- **"전부 0" 갈래는 두지 않는다.** 우리 코퍼스에서 그 갈래가 더 막는 가짜는 **0개**이고(11 ≥ 10 —
  `젤라토프로틴추출물` 은 `추출물` df 1,341 때문에 전부-0 이 아니다) 진짜 별칭 2개를 막는다. ydc 는 그
  갈래를 두고 `재도포` 가 막히는 것을 옳다고 봤지만, 우리는 같은 이득 0 · 손해 2 다.
- **토큰이 0개인 질의는 df 로 판정하지 않는다 — 통과시킨다.** `땀에`·`톤 업`·키릴 표기가 그 갈래이고,
...
- **df 는 색인 축에서 읽는다**(`bm25.tokenize` · `Index.postings`) — 질의 불용어 목록(포크 #46)을 타지
...
재는 그 주제 별칭 67개)를 통과시키면 `bm25` 로 가는 질의가 **2개, 옳게 보낸 것은 0개**다 — `밀려` 가 브랜드
...
| 열려 있는 신호 둘(브랜드 950표기 + SPF/PA)만 보는 라우터가 주제 별칭 67개에서 `bm25` 로 보내는 질의 | **2**, 그중 옳은 것 **0** |
