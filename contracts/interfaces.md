...
중 **분기도 명부도 없다** — 리뷰 수집이 최신 편향이라(2026년 90%) 분기로 쪼개면 "2026년 폭증"이 나오고
그것은 수집 방식의 산물이며, 플랫폼 속성 평가의 관측 창은 며칠뿐이다. 성분 축에는 `topic_key` 축 자체가
없다(`INGREDIENT_KEYS` 는 `aspect_lexicon(ruleset='retrieval-topic')` 의 레지스트리가 아니다). 자리를
만드는 것은 추가만의 범위가 아니라 저장된 비율의 뜻을 바꾸는 일이므로 산출은 표가 아니라 **답**이고
...
- **`share_pct` 가 NULL 인 행은 들지 않는다.** 그 소스는 비중 대신 가중치(`score`)를 싣는데, 가중치와
  백분율은 다른 단위라 섞어 평균 내면 아무것도 보여 주지 않고 틀린다(`review_topic.score` 의 DDL 주석이 그 문장을 이미 든다). 전량에서 그 소스는 10,764행 · 35제품이다.
- **시점별 스냅샷이라 (제품, 선택지)별 최신 `captured_at` 한 행만 쓴다.** 같은 선택지가 수집 시점마다 한
  행씩 쌓이고, 속성 평가는 리뷰가 쌓여야 바뀌므로 시점 간 값이 거의 같다. 전부 세면 제품 수가 시점 수만큼
  부풀려진다.
- 극성은 선택지 문구가 정한다(`NEUTRAL_HINTS` → `NEGATIVE_HINTS` → 나머지가 긍정). 긍정률 = 한 제품·한
  `topic_group` 안에서 긍정 선택지가 차지하는 비중이고, 중립만 있으면 0 이다.
...
- **그래서 기계 게이트는 사람이 한 번 읽어 금지한 목록 하나다.** 어느 키든 `DENIED_NAMES`
  (트라이에톡시카프릴릴실레인 · 트리에톡시카프릴릴실란 · 레티놀)를 잡으면 `key_mismatch` 이고 종료 코드
  1 이다(§entrypoints). 잡은 행이 0인 것은 오매칭이 아니라 부재이므로 통과다 — `레티날`·`PDRN`·`엑소좀`·
  `트라넥삼산` 이 우리 표에서 그 자리다.
- **그 목록이 못 보는 자리는 테스트가 진다.** 키가 무엇을 잡는지(키별 고유 성분명 집합)를
  `tests/test_crosscheck_keys.py` 가 우리 표에서 실제로 뜬 이름으로 걸어 둔다 — 코퍼스가 자라 새 물질이
  어떤 키에 들어오면 그 줄이 먼저 깨지고 사람이 다시 읽는다. 종료 코드로는 이 일을 할 수 없다(자라는
  코퍼스에서 새 이름은 평상 상태다).
...
| `SUN_SHARE_LOW` | `25` | 선크림 문맥 비율이 이 아래면 그 수를 "선크림 담론" 으로 읽지 말라고 말한다. ydc 실측 PDRN 187/1,522(12.3%)와 우리 149/933(16.0%)이 둘 다 이 아래다 | 채택. 적합값이 아니라 읽기 기준이다 — 4분의 1 은 "대체로 선크림 얘기다" 라고 말할 수 있는 최소선이고, 전량에서 열 성분이 **전부** 이 아래라 이 열은 지금 경고 하나를 뜻한다 |
...
- **규칙** 열넷(`ranks` · `positive_rate` · `polarity` 다섯 · 평가 해석 넷)에 같은 입력을 먹여 답을 비교한다.
