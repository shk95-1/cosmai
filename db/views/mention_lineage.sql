...
--
-- 문장은 **발췌만** 나간다. anon 은 지금 유튜브 댓글 원문은 보지만 리뷰 본문은 못 본다
-- (db/grants/postgrest_anon_needs.sql:3 이 mention 표를 일부러 막는다). 이 뷰가 그 선을 무너뜨리지 않게
-- 120자에서 자르고 전문 길이를 나란히 둔다 -- 근거로 쓰기엔 충분하고 전문 재구성은 안 된다
-- (사용자 결정 2026-08-27). 원문 컬럼(sentence·body·text)은 이름조차 나가지 않는다.
--
-- 필터 없이 부르면 need_mention 전부를 훑는다. 화면은 언제나 칸의 축으로 좁혀 부르고
-- (PGRST_DB_MAX_ROWS=1000, mention_id 정렬로 이어 읽는다), 이 뷰는 그 쓰임에 맞춰져 있다.
--
