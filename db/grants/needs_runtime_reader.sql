...
-- 읽지 않는 것(근거): needs_runtime 은 trend_radar.run/fetch_log/run_source 에 직접 닿지 않는다 --
-- 셋 다 needs_owner 가 뷰를 통해서만 읽는다 · trend_radar.review_summary · tubedepth 전부(운영 뷰의
-- youtube 팔은 3단계에서 빠졌다, contracts/entrypoints.md) · tubedepth.channel_snapshots ·
-- tubedepth 수집기 내부 상태 테이블 · cosmai.* 전부.
