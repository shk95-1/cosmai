...
2026-08-23 재구성에서 히스토리 비공유로 시작한 모노레포다. 현재 개발·배포는 `shk95-1/cosmai` 한 저장소에서 진행한다. 임시 포크 `cosmai-import-ydc`는 흡수 후 과거 코드와 이슈 처분 기록으로 보존한다(#322). 이전 저장소는 재사용·참고 자산이며 별도 개발 큐가 아니다. 실제 archive 상태는 저장소마다 다르다. 계약·추가형 DDL·이관 출처는 계속 보존한다.

제품 목표는 #321의 니치 화장품 요구 발굴과 근거 있는 개발 브리프다. 사용자 결정에 따라 #181에서 이전에 실행하던 Docker 컨테이너 15개 전체를 개발·로컬 운영환경으로 복원하고, 남은 이슈 계획을 해결한 다음 서비스를 정리한다. 현재 DB 실행 방식과 승인 경계는 `STATE.md`, 실행할 작업은 이슈가 기준이다.
