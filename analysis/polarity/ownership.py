...
# 2026-08-24 홀드아웃에서 gemma4 가 규칙을 넘었다 (interfaces.md §LLM 실측). #31 이 "카테고리 있는
# 리뷰 전부"로 4 를 정의해, 선블록에 이어 나머지 26개 카테고리를 여기 한 번에 등록한다 — 등록이 패스보다
# 먼저다(등록 없는 scope 를 gemma4 로 돌리면 CLI 가 exit 2 로 거절한다, unready() 참고). 등록만 하고
# 패스를 아직 안 돌린 scope 는 규칙 크론이 더 이상 갱신하지 않으므로 기존 규칙 라벨 그대로 굳는다
# (사라지지는 않는다) — 그 공백은 순차 패스가 끝나면 닫힌다.
_GEMMA4_2026_08_24 = "llm-ollama-gemma4:latest-fs2-20260824"
OWNERS: Mapping[str, str] = MappingProxyType(
    {
        scope: _GEMMA4_2026_08_24
        for scope in (
            "선블록",
            "에센스",
            "쿠션",
