...
class EvalSet:
    ...
    checks: tuple[Check, ...]
    ref_prefix: str = ""
    roster: str = ""  # eval/ 아래 CSV — ref 접두로 가릴 수 없는 셋의 명부
    roster_column: str = ""

    def refs(self) -> frozenset[str] | None:
        """명부가 있는 셋만 ref 목록을 돌려준다 — 없으면 ref_prefix 가 고르는 기준이다."""
        if not self.roster:
            return None
        with (EVAL_DIR / self.roster).open(newline="", encoding="utf-8") as f:
            return frozenset(row[self.roster_column] for row in csv.DictReader(f))
...
BASELINES: tuple[EvalSet, ...] = (
    ...
    # 기준선이 블라인드 쪽만 가리키므로 명부는 그 셋의 원본 CSV 에서 읽는다.
    EvalSet(
        task="wish_class",
        name="P9 blind60_v2",
        split="holdout",
        checks=(Check("P:a", 0.90),),
        roster="wish/blind60_v2.csv",
        roster_column="comment_id",
    ),
