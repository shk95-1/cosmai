"""Reads one version of needs.entity_lexicon / needs.aspect_lexicon and returns a compiled dictionary.

The regex vocabulary (particles, product words, discourse markers, wish markers) is carried over from rule
v2.2 of slice-p3 link_brands.py and slice-p1 aspects_generic.py as it was. The read runs as the
needs_runtime role (search_path=needs), so the table names are not qualified.
"""
...
# The allowed particle tails and the product words. The boundary rule of surface_re has the same shape as
# the RX of slice-p3.
PARTICLES = (
...
# The markers that pick a candidate sentence. Unlike an aspect pattern these are not in the dictionary table
# but attached to the rule version (slice-p1).
DISCOURSE_MARKERS = (
...
# The fallback pattern for when there is no surface at all: an empty alternation matches at every position
# and a run without a dictionary becomes all hits.
NEVER = re.compile(r"(?!)")
...
def compile_lexicon(surfaces: Sequence[EntitySurface], version: int) -> Lexicon:
    """Rows -> a compiled dictionary. A dictionary can be built without a DB, so the rules of the linker and
    the extractor can be checked as pure functions."""
