...
|---|---|
| `product_ref` | the canonical ref (`needs.product_ref`), when it is known — otherwise leave it empty and fill the next two |
| `brand` · `name` | the product as a person names it; the tool resolves the pair against `needs.product_ref` |
...
Either `product_ref` or `brand` + `name` is required, and so is `launch_month`; a row missing both
sides is refused by the loader rather than silently scored as a miss.
