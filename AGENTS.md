...
- When two or more open at once in one channel (a wave), stand up a `wave/<channel>` branch, rebase the issue branches onto it, and merge that branch into main once at the end of the wave. Re-run the suite after a rebase. After the merge, delete the wave branch locally and on origin — the channel's next wave is cut fresh from main. Outside a wave: issue branch → main. A worker pushes every time the suite is green.
