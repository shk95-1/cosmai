...
- When two or more open at once in one channel (a wave), stand up a `wave/<channel>` branch, rebase the issue branches onto it, and merge it into main once at the end of the wave; re-run the suite after a rebase. An issue started in that channel while `wave/<channel>` exists joins the wave: rebased onto it, merged with it, never to main on its own. After the merge, delete the wave branch locally and on origin — the next wave is cut fresh from main. Outside a wave: issue branch → main. A worker pushes when the suite is green.
